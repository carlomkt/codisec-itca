...
import jwt from 'jsonwebtoken'; // Added JwtPayload import
...
function permissionRequired(permissionName) {
    return (req, res, next) => {
        const user = req.user;
...
if (!fs.existsSync(itcaDir))
    fs.mkdirSync(itcaDir);
...
app.post('/api/itca/import', authRequired, permissionRequired('page:actividades'), (req, res) => {
    ...
    if (!workbookBase64)
        return res.status(400).json({ error: 'workbookBase64 requerido' });
    try {
        const base64 = workbookBase64.split(',').pop();
...
async function seedCatalog() {
    async function ensure(type, values) {
