...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="sticky top-0 z-20 h-14 border-b bg-[#003366] text-white flex items-center justify-between px-4 shadow">
        <div className="flex items-center gap-3">
          <img src="/images/logo.png" alt="CODISEC" className="h-8 w-auto" />
          <span className="font-semibold">CODISEC Chorrillos</span>
        </div>
        <button className="md:hidden px-3 py-2 rounded bg-[#0077cc]" onClick={() => setOpen(!open)}>Menú</button>
        <nav className="hidden md:flex items-center gap-4 text-sm">
          ...
          <Link to="/config/catalog" className="hover:underline">⚙️</Link>
        </nav>
      </header>
      <div className="flex">
